...
# URL do IP Webcam do celular (coloque o IP correto do seu celular)
...
if not ret or frame is None:
    print("Não foi possível capturar frame inicial. Verifique o IP da câmera.")
    exit()

h, w = frame.shape[:2]
roi_cx, roi_cy = w // 2, h // 2  # centro inicial do ROI
...
while True:
    ...
    x2 = min(w, int(roi_cx + ROI_SIZE // 2))
    y2 = min(h, int(roi_cy + ROI_SIZE // 2))
    roi = frame[y1:y2, x1:x2]

    # --- DETECÇÃO DA LINHA PRETA ---
    gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
    _, black_line = cv2.threshold(gray, 100, 255, cv2.THRESH_BINARY_INV)
    contours, _ = cv2.findContours(black_line, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
